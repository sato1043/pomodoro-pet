...
| settings-ipc.spec.ts | 13 | electronAPI存在・settings.json永続化（タイマー/BG/天気/雲量/シーンプリセット/テーマ）・再起動復元 |
| registration.spec.ts | 8 | ライセンス/アップデート/shell API存在・Registerリンク表示・登録パネル表示/CloseButton復帰/空キーエラー/入力・REGISTRATION_GUIDE読み込み |
| weather-panel.spec.ts | 13 | パネル開閉・シーンプリセット切替・天気タイプ/時間帯切替・雲量・リセット・スナップショット復元（天気+プリセット）・排他表示 |
...
| WorldMapModal — E2E未実装（検証可能） | 9項目 |
| 天文シミュレーション — WebGL依存（E2E困難） | 3項目 |
| 手動テスト手順 | 9項目（D-5-1〜D-5-9） |

E2Eテストは **UIレイアウト・パネル制御・設定永続化・タイマーフロー（完走含む）・全フェーズ遷移・プリセット切替・シーンプリセット切替・感情パラメータ・感情パラメータ永続化（emotion-history.jsonファイル生成・再起動復元）・感情インジケーター（表示/非表示/opacity整合/ライセンス制限）・感情推移グラフ（表示・データなし/あり・期間切替）・プロンプト入力・インタラクションロック・統計値・affinity永続化・バックグラウンドタイマー・ライセンスAPI存在確認・RegistrationDialog UI操作・カスタムタイトルバー（ウィンドウ操作ボタン・frame: false確認）・バイオリズムON/OFF（ライセンスモード連動）** をカバーしている。未カバー項目はThree.jsキャンバス内操作（バイオリズムブースト含む）、サーバー通信依存、Web Audio API / WebGL / OS通知依存であり、E2Eテストの技術的限界に起因する。WorldMapModalはSVG+DOMで構成されプリセット選択・設定永続化等9項目がE2E検証可能だが、SVG座標変換やrAFアニメーション等5項目は自動検証困難なため手動テスト手順（D-5-2〜D-5-7）で補完する。Phase 5.5の純粋関数ロジックはユニットテストでカバー済み（Terminator 8テスト、ClimateData 21テスト、normalizeLon 11テスト、coastline-path 16テスト、CelestialTheme 27テスト、WeatherDecision 22テスト、SolarPosition 11テスト、Kou 17テスト、ClimateGridAdapter 7テスト）。ライセンス判定ロジック（LicenseState.ts）はユニットテスト（66テスト）で全パターンカバーされている。
...
| # | 項目 | 検証方法 |
|---|------|----------|
| 1 | KouDisplay表示 | `[data-testid="kou-display"]`の存在とテキスト内容 |
| 2 | WorldMapModal開閉 | LocationButton→モーダル開→戻るボタン→モーダル閉 |
| 3 | プリセット都市ボタン選択 | モーダル内プリセットボタンクリック→座標情報テキスト更新（`coord-info`クラス要素） |
| 4 | プリセット都市ピン選択 | モーダル内SVG都市ピンクリック→座標情報テキスト更新 |
| 5 | Set Location→設定適用 | 「Set Location」クリック→モーダル閉→climate設定永続化 |
| 6 | autoWeather設定永続化 | settings-ipc.spec.tsにautoWeather=true→再起動→復元の検証追加 |
| 7 | climate設定永続化 | settings-ipc.spec.tsにclimate→再起動→復元の検証追加 |
| 8 | IDL（日付変更線）パス存在 | SVG内にstrokeDasharray付きpathが存在すること |
| 9 | 陸地パス存在 | SVG内にfill="#2a4a2a"のpathが存在すること |
...
| Kou.ts | 17 | 七十二候 |
| ClimateGridAdapter.ts | 7 | 気候グリッド補間 |
...
**操作手順**:
1. アプリ起動
2. 画面上のKouDisplay領域を確認

**確認内容**:
- 現在の七十二候名（日本語+英語）が表示される
...
##### D-5-8. 天文計算ベースのライティング変化

**前提**: EnvironmentSimulationServiceがmain.tsに統合済み、autoTimeOfDay=true
