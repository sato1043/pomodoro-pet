...
| 2 | WorldMapModal開閉 | LocationButton→モーダル開→戻るボタン→モーダル閉 | 未実装 |
| 3 | プリセット都市ボタン選択 | モーダル内プリセットボタンクリック→座標情報テキスト更新（`coord-info`クラス要素） | 未実装 |
| 4 | プリセット都市ピン選択 | モーダル内SVG都市ピンクリック→座標情報テキスト更新 | 未実装 |
| 5 | Set Location→設定適用 | 「Set Location」クリック→モーダル閉→climate設定永続化 | 未実装 |
