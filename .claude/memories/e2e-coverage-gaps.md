...
| registration.spec.ts | 8 | ライセンス/アップデート/shell API存在・Registerリンク表示・登録パネル表示/←ボタン復帰/空キーエラー/入力・REGISTRATION_GUIDE読み込み |
| weather-panel.spec.ts | 22 | environmentシーン遷移・シーンプリセット切替・天気タイプ/時間帯切替・雲量・リセット・スナップショット復元（天気+プリセット）・environmentシーンでfreeUI非表示・autoWeather排他選択動作・autoWeather時Weather操作でauto解除+Cloud行disabled・autoWeatherとautoTimeOfDay独立動作・WeatherPanel Scene行Locationボタン表示・WeatherPanel→WorldMapModal→戻りでWeatherPanel復帰・Time手動→autoWeather→再解除のフロー・KouSelectorはenvironmentシーンのみ表示/Autoトグル/リスト手動選択Auto解除/日付範囲表示 |
| world-map-modal.spec.ts | 7 | environment内LocationボタンからWorldMap開閉・戻るでWeatherPanel復帰・プリセット都市選択→座標更新・ケッペン気候区分表示・複数都市での分類表示・Set Location→WeatherPanel復帰 |
| button-visibility.spec.ts | 5 | 初期全表示・設定/統計パネル時の排他制御・environmentシーン遷移時freeUI非表示・順次開閉復帰 |
...
E2Eテストは **UIレイアウト・パネル制御・設定永続化・タイマーフロー（完走含む）・全フェーズ遷移・プリセット切替・シーンプリセット切替・感情パラメータ・感情パラメータ永続化（emotion-history.jsonファイル生成・再起動復元）・感情インジケーター（表示/非表示/opacity整合/ライセンス制限）・感情推移グラフ（表示・データなし/あり・期間切替）・プロンプト入力・インタラクションロック・統計値・affinity永続化・バックグラウンドタイマー・ライセンスAPI存在確認・RegistrationDialog UI操作・カスタムタイトルバー（ウィンドウ操作ボタン・frame: false確認）・バイオリズムON/OFF（ライセンスモード連動）・Autoテーマ選択+モード相互切替+永続化+再起動復元・autoWeather排他選択動作+disabled連動+永続化+再起動復元・autoWeather/autoTimeOfDay独立動作・environmentシーン遷移+freeUI排他・WeatherPanel Scene行Locationボタン表示+WorldMapModal復帰フロー（environmentシーン内ビュー切替）・Time手動選択→autoWeather→再解除フロー** をカバーしている。未カバー項目はThree.jsキャンバス内操作（バイオリズムブースト含む）、サーバー通信依存、Web Audio API / WebGL / OS通知依存であり、E2Eテストの技術的限界に起因する。WorldMapModalはSVG+DOMで構成され13項目中8項目（モーダル開閉・プリセット選択・座標更新・ケッペン気候区分表示・Set Location適用・autoWeather永続化等）がE2Eテスト実装済み。残り5項目が未実装。SVG座標変換やrAFアニメーション等5項目は自動検証困難なため手動テスト手順（D-5-2〜D-5-7）で補完する。Phase 5.5の純粋関数ロジックはユニットテストでカバー済み（Terminator 8テスト、ClimateData 40テスト、normalizeLon 11テスト、coastline-path 16テスト、CelestialTheme 29テスト、WeatherDecision 22テスト、SolarPosition 11テスト、Kou 21テスト、ClimateGridAdapter 7テスト、useResolvedTheme 20テスト、AppSettingsService loadFromStorage 9テスト、EnvironmentSimulationService 40テスト）。ライセンス判定ロジック（LicenseState.ts）はユニットテスト（66テスト）で全パターンカバーされている。

### D. Phase 5.5 天文シミュレーション・世界地図UI

Phase 5.5の天文計算ベース環境シミュレーションとWorldMapModalはSceneEnvironment.tsx（独立AppScene）に統合済み。WeatherPanel・KouSelector・WorldMapModalはenvironmentシーン内で表示される。E2Eテストで検証可能な項目と、技術的制約で困難な項目に分類する。
...
| 11 | ~~LocationButton常時表示~~ | ~~LocationButtonはfreeモードから削除済み（environmentシーンに移行）~~ | **削除** |
| 12 | WeatherPanel Scene行Locationボタン表示+WorldMapModal復帰フロー | weather-panel.spec.tsで`weather-location`存在確認+WeatherPanel→WorldMapModal→戻りでWeatherPanel復帰（environmentシーン内ビュー切替） | **実装済み** |
...
**前提**: EnvironmentSimulationServiceがmain.tsに統合済み（autoWeather状態に関わらず常時稼働）。KouSelectorはenvironmentシーン内でのみ表示される。

**操作手順**:
1. アプリ起動
2. WeatherButton（雲アイコン）をクリックしてenvironmentシーンに遷移
3. ウィンドウ上端中央のKouSelector（`data-testid="kou-selector"`）を確認
4. Autoボタンがactive状態であることを確認
...
**確認内容**:
- KouSelectorはenvironmentシーン内のweatherビューで表示される（WorldMapビュー時は非表示）
...
**操作手順**:
1. environmentシーンに遷移（WeatherButtonクリック）
2. WeatherPanel Scene行のLocationボタンをクリック
3. 全画面の世界地図モーダルが開く
...
**確認内容**:
- WorldMapが閉じてWeatherPanelに復帰する（environmentシーン内のビュー切替）
...
- environmentシーンのWorldMapで地域を変更した場合、即座にタイムゾーンのisDaytimeが再評価される
