...
- opacityフェード（fadeIn/fadeOut）でテーマ遷移と同期した滑らかなエフェクト切替
- 雲の密度変更時クロスフェード（退場バッチ方式、2000ms）
...
- 5.5h: 七十二候セレクタ（KouSelector、ウィンドウ上端ドロップダウン+Autoボタン、候の手動/自動切替）
...
### ~~環境映像（背景動画）~~ — 不採用
- Phase 5.5の天文計算ベース環境シミュレーション（動的ライティング・空色・天気エフェクト・シーンプリセット）により、背景動画なしで十分な環境表現を実現済み。動画アセットの容量・ライセンス管理コストに見合わないと判断

### ~~設定・統計のクラウド保存~~ — 不採用
- デスクトップ専用アプリで複数デバイス同期のユースケースが希薄。Firestore永続化・同期衝突処理・プライバシーポリシー更新等の実装・運用コストに見合わないと判断
- 代替として「設定・統計のエクスポート/インポート」で対応する

### ~~設定・統計のエクスポート/インポート~~ — 実装済 (v0.9.0)
- 設定（settings.json）、統計（statistics.json）、感情履歴（emotion-history.json）をローカルJSONファイルとしてエクスポート/インポート
- 設定パネルのData行にExport/Importアイコンボタン配置
- バージョン互換性検証（メジャーバージョン一致チェック）、確認ダイアログ、deviceId/license情報保持マージ
- インポート成功時はアプリ再起動（app.relaunch→app.exit）

### ~~リリースバージョンとベータ/アルファバージョンの機能セット切替~~ — 完了
- `ReleaseChannel`型（stable/beta/alpha）と`FEATURE_CHANNEL`マップで各機能の公開チャネルを定義
- `isFeatureEnabled(mode, feature, channel?)`でチャネル×ライセンスモードの2軸判定
- `VITE_RELEASE_CHANNEL`環境変数でビルド時にチャネルを決定（デフォルト: stable）
- `ChannelBadge`コンポーネントでbeta/alpha時に左下にバッジ表示
- `LicenseContext`に`releaseChannel`を追加、`canUse()`にチャネル判定を統合
- alpha機能はregistered限定。既存全13機能はstable
- 自動アップデートのチャネル分離を実装済み（GCPバックエンドの`releases/{channel}`コレクション、heartbeat APIのchannelフィールド、CI/CDプレリリースタグ対応）
- 詳細: [release-infrastructure.md](release-infrastructure.md)

### 天球に恒星を表示
- 実恒星カタログ（Hipparcos等）から明るい星（視等級4.5以下、約1500個）を天球座標系に配置
- 赤経/赤緯 → 地方恒星時(LST)で時角変換 → 方位角/高度 → celestialToSceneで統一変換
- 天文的にリアルな要素: 日周運動（東→西移動）、季節変化（見える星座の変化）、薄暮消滅（昼間非表示）
- 視等級に基づくサイズ/輝度差
- THREE.Points + BufferGeometryで描画（軽量）
- astronomy-engineのSiderealTime関数でLST計算
- 実装レイヤー: domain(`StarCatalog.ts`, `StarPosition.ts`) + infra(`StarEffect.ts`)
- 詳細設計は [celestial-mapping-design.md](celestial-mapping-design.md) の天球座標系を基盤とする

### コードベースのリファクタリング
- 重複コードの削除
- テストカバレッジの向上
- 設計点検（クリーンアーキテクチャ準拠、依存方向、レイヤー責務の見直し）
...
- [x] GitHub Actions: リリースワークフロー構築 → `.github/workflows/release.yml` 作成済み。セットアップ手順は [release-infrastructure.md](release-infrastructure.md) を参照
- [x] package.jsonのbuild.publishのowner/repoをプレースホルダーから実際の値に変更
- [x] itch.io butler CLI自動アップロード統合 → `release.yml`にsetup-butler Action + butler pushステップ追加。GitHub Secret `BUTLER_CREDENTIALS`設定済み
- [x] GitHub Actions Node.js 24対応: actions/checkout v4→v6、actions/setup-node v4→v6 更新済み
- [ ] GitHub Actions Node.js 24対応（上流待ち）: jdno/setup-butler v1、softprops/action-gh-release v2 が未対応。2026-06-02までに上流が対応しなければ `ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION=true` で一時回避が必要
- [x] productionビルド動作確認 → v0.11.1インストーラーをWindows上で起動確認済み（2026-03-15）

#### 販売ページ・ドキュメント
- [x] 特定商取引法に基づく表示の内容の準備 → 販売者: Updater LLC. / 住所: バーチャルオフィス（日本橋蛎殻町）
- itch.io販売ページの作成・公開
  - [x] Projectページ作成
  - [x] Tax Interview提出
  - [x] スクリーンショット、説明文（英語/日本語）、価格設定 → コンテンツ準備済み。[itchio-page-content.md](2026-03-15_PomodoroPet/itchio-page-content.md) にテキスト、スクリーンショット8枚を `.claude/memories/2026-03-15_PomodoroPet/` に保存。itch.ioへの手動設定
  - [x] 返金不可の明記（購入ボタン付近に目立つ形で）  → テキスト準備済み、itch.ioへ記載
  - [x] EULA全文の掲載 + 「By purchasing, you agree to the EULA」の文言 → テキスト準備済み、itch.ioへ記載
  - [ ] 特定商取引法に基づく表示の記載 → テキスト準備済み、itch.ioへの記載は未実施
  - [x] SmartScreen 警告の回避手順を注記する → テキスト準備済み、itch.ioへの記載は未実施
- [x] 登録ガイド（REGISTRATION_GUIDE.txt）のお問い合わせ先メールアドレスを本番用に差し替え → support@updater.cc
- [x] 登録ガイド（REGISTRATION_GUIDE.txt）の購入URLをダミー（https://www.updater.cc）から本番URLに差し替え → https://updaterllc.itch.io/pomodoropet
...
- [x] Firestoreリリースバージョン更新 → `releases/stable = 0.11.1`（2026-03-15）
- [x] GitHub Secrets設定 → `ASSETS_DEPLOY_KEY`, `VITE_HEARTBEAT_URL`, `VITE_STORE_URL`（https://updaterllc.itch.io/pomodoropet）, `BUTLER_CREDENTIALS` 設定済み
- [x] register API に itch.io download key 検証を実装 → itch.io API（game_id=4370345）で新規キー登録時にdownload keyの有効性を検証。deploy.shに`ITCHIO_API_KEY`シークレット+`ITCHIO_GAME_ID`環境変数を追加。要再デプロイ
- [ ] デバッグ用キー発行の制限、または無効化機能
