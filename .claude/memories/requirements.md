...
- Weather行: 天気4種アイコン + Auto（排他選択。Autoクリック→auto有効、天気アイコンクリック→auto解除）
...
##### 13.7.8 七十二候UI表示
- `KouDisplay`コンポーネント（createPortalでdocument.bodyに描画）
- 右下に候名を表示（solarTermName + phase + nameJa + readingJa）
- 候変更時のフェードアニメーション: opacity 0→0.8→0.4（setTimeoutで制御）
- pointer-events: none（UIクリック透過）
...
- 手動操作（setManualTimeOfDay/setManualWeather/setAutoWeather等）時のテーマ遷移は1.5秒。通常30秒間隔の天体更新時は30秒遷移
...
- KouDisplayはcurrentKou !== null時に表示（autoWeather非依存）
