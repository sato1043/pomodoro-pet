...
### 10A. データエクスポート/インポート — 実装済

#### 10A.1 概要
設定（settings.json）、統計（statistics.json）、感情履歴（emotion-history.json）をローカルJSONファイルとしてエクスポート/インポートする。PC買い替え時やバックアップ用途を想定。registeredライセンス専用機能（`dataExportImport` FeatureName）。trial/expired/restrictedでは非表示。

#### 10A.2 エクスポート
- 設定パネル展開時の「Data:」行にExportアイコンボタンを配置
- クリックでファイル保存ダイアログを表示（デフォルトファイル名: `pomodoro-pet-backup-YYYY-MM-DD.json`）
- エクスポートファイル形式: `{ version, exportedAt, settings, statistics, emotionHistory }`
- settings/statistics/emotionHistoryは各JSONファイルの内容をそのまま埋め込む

#### 10A.3 インポート
- 設定パネル展開時の「Data:」行にImportアイコンボタンを配置
- クリックでファイル選択ダイアログを表示（JSONフィルター）
- バリデーション: メジャーバージョン一致・必須フィールド存在チェック
- 確認ダイアログ表示（Cancel / Import & Restart）
- インポート時にdeviceId/downloadKey/jwtは現在の値を保持（ライセンス情報を上書きしない）
- 成功時はアプリを再起動（`app.relaunch()` → `app.exit(0)`）

#### 10A.4 IPC
- `data:export` — ファイルダイアログ→エクスポート処理→結果返却
- `data:import` — ファイルダイアログ→バリデーション→確認→インポート→アプリ再起動
...
- Scene行: meadow/seaside/parkの3アイコンボタン（山/波/木のSVGアイコン）+ 右端にLocationボタン（地球アイコン、`weather-location`）。クリックでWorldMapビューに遷移。WorldMapのclose→weatherビューに復帰（environmentシーン内のビュー切替）
...
- environmentシーン遷移時: EnvironmentCoordinatorがWeatherPreviewOpenイベント発行。カメラをふれあいモード位置に後退、キャラクターmarch-cycleプリセット
...
- 夜間: 月齢+月高度→月光色・月光強度。月光ブースト（nightExposure: lerp(0.08, 0.45)、nightAmbientIntensity: lerp(0.08, 0.40)）
- 夜間: 地面色をmoonBrightness×0.3でMOONLIGHT_COLOR(0x8899bb)方向にブレンド
...
- EnvironmentThemeParamsに月データ5フィールド: moonPosition, moonPhaseDeg, moonIllumination, moonIsVisible, moonOpacity。テーマ遷移パイプライン（30秒lerp）で滑らか補間
- 旧THEME_TABLEルックアップに代わる連続関数方式

##### 13.7.3 天体→光源の向き
- 日中: 太陽方位角・高度角→DirectionalLight方向ベクトル
- 夜間: 月方位角・高度角→DirectionalLight方向ベクトル。月光intensity係数=illuminationFraction × 0.8
...
- 3段構成:
  - Row 1: `season`ラベル + `#候番号 | 日付範囲`
  - Row 2: 候の英語名（nameEn）
  - Row 3: Autoアイコン（時計SVG）+ リストアイコン（リストSVG）
- Row 3の下に詳細表示: 節気+フェーズ和名（fontSize 22） / 候名和名（fontSize 22） / 読み仮名（全角カッコ書き） / 説明文 / λ=黄経
- リストアイコンクリックで全72候のフルスクリーンオーバーレイリストを表示
  - テーブル形式: #番号、日付範囲、節気、候名（候名列は右端まで伸長）
  - テーブルヘッダは固定、候リストはスクロール可能（3:1比率でリスト:詳細パネル）
  - リスト下部に詳細パネル（背景rgba(0,0,0,0.7)）: #/日付範囲 → 英語名2行 → 和名2行 → 読み → 説明 → λ°
  - 2クリック選択: 1回目=プレビュー、2回目=確定。確定でautoKouがfalseに切り替わりリストが閉じる
  - 閉じるボタンはWeather戻るボタンと同位置・同サイズ（bottom:168, left:10, 48x48）
- Autoアイコン: inactive=rgba(255,255,255,0.7)、active=textMuted（#aaa）
- `KouDefinition`に`solarTermNameEn`フィールド（24節気の英語名）を追加
- Auto時（`autoKou: true`）: currentKouに逐次追従
- 手動時（`autoKou: false`）: リストから選択した候indexが`manualKouIndex`として`EnvironmentSimulationService.setManualKou()`に伝搬
- リストからの手動選択で`autoKou`が自動的にfalseに切り替わる
...
- ロケーション設定（WorldMapModal）はautoWeather非依存で常に利用可能（environmentシーン内のLocationボタンからアクセス）
- KouSelectorはenvironmentシーンのweatherビュー内で表示（WorldMapビュー時は非表示）

##### 13.7.11 3D月オブジェクト
- SphereGeometry(1.0, 32, 32)をスケール18.0で天球上に配置（距離300、fog無効）
- 月位相テクスチャ: Canvas 128×128でgenerateMoonPhasePixels()により動的生成。terminator曲線で満ち欠け+地球照（earthshine）効果
- グロー: BackSide半透明球（scale 1.3×月球体、色0xaabbdd）
- 水平線フェード: altitude -2°〜+5°で0→1
- 天気減衰: MOON_WEATHER_DIMMING（sunny=1.0, cloudy=0.4, rainy=0.1, snowy=0.1）
- 表示位置リマップ: azimuthを北方向中心±25°に補正（東→画面左、西→画面右）。altitudeを22°〜36°にリマップし画面上半分に配置
- MoonPhase.ts（ドメイン層純粋関数）: Three.js非依存のテクスチャ生成
- MoonEffect.ts（インフラ層）: Three.jsメッシュ管理、applyThemeToScene経由で更新

##### 13.7.12 月高度UI
- 環境パネルのTime行の下にMoon行を配置（Horizon/Low/Mid/High/Autoの5ボタン）
- 高度プリセット: Horizon=10°, Low=15°, Mid=25°, High=33°
- 高度→月齢連動: 半月(phaseDeg=90)→満月(phaseDeg=180)
- 高度→方位角連動: 東寄り(150°)→西寄り(210°)で太陽と同じ左→右の動き
- Auto: astronomy-engineのリアルタイム月位置を使用
...
- SceneFree: StatsButton/FureaiEntryButton/WeatherButton（environmentシーン遷移）をcanUseで表示制御
