...
test('KouSelectorが表示される', async () => {
  const { page } = app

  const selector = page.locator('[data-testid="kou-selector"]')
  await expect(selector).toBeVisible()

  const select = page.locator('[data-testid="kou-select"]')
...
test('KouSelectorドロップダウン手動変更でAutoが解除される', async () => {
  const { page } = app
  ...
  // ドロップダウンで手動選択（index=6: 立春初候「東風解凍」）
...
test('KouSelectorドロップダウンに日付範囲が表示される', async () => {
  const { page } = app

  const select = page.locator('[data-testid="kou-select"]')
  // 最初のoptionのテキストが日付範囲フォーマット（# 1 | M/D - M/D）を含む
