...
describe('CyclePlan', () => {
  describe('buildCyclePlan', () => {
    it('Sets=1: [work, congrats, break]', () => {
      ...
      expect(plan[1]).toEqual({ type: 'congrats', durationMs: CONGRATS_DURATION_MS, setNumber: 1 })
      expect(plan[2]).toEqual({ type: 'break', durationMs: 5 * 60000, setNumber: 1 })
    })

    it('Sets=2: [work, break, work, congrats, long-break]', () => {
      ...
      expect(plan[3]).toEqual({ type: 'congrats', durationMs: CONGRATS_DURATION_MS, setNumber: 2 })
      expect(plan[4]).toEqual({ type: 'long-break', durationMs: 15 * 60000, setNumber: 2 })
    })

    it('Sets=4: 最終work直後にcongrats、末尾にlong-break', () => {
      ...
      expect(plan[7]).toEqual({ type: 'congrats', durationMs: CONGRATS_DURATION_MS, setNumber: 4 })
      expect(plan[8]).toEqual({ type: 'long-break', durationMs: 15 * 60000, setNumber: 4 })
    })
  ...
  describe('cycleTotalMs', () => {
    ...
    it('Sets=2: work*2 + break + congrats + long-break', () => {
      const config = createConfig(25 * 60000, 5 * 60000, 15 * 60000, 2)
      const plan = buildCyclePlan(config)
      expect(cycleTotalMs(plan)).toBe(70 * 60000 + CONGRATS_DURATION_MS)
    })

    it('Sets=4: work*4 + break*3 + congrats + long-break', () => {
      const config = createConfig(25 * 60000, 5 * 60000, 15 * 60000, 4)
      const plan = buildCyclePlan(config)
